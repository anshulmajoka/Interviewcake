package interviewcake.arrays.orderchecker;
...
public class Solution {
    ...
    public static boolean isFirstComeFirstServed(int[] takeOutOrders, int[] dineInOrders, int[] servedOrders) {
        ...
        for (int i = 0; i < servedOrders.length; i++) {
            ...
            if (takeOutOrders.length > t && takeOutOrders[t] == servedOrders[i]) {
                t++;
            } else if (dineInOrders.length > d && dineInOrders[d] == servedOrders[i]) {
                d++;
            } else {
                return false;
            }
